...
def test_openai_uses_hosted_url_when_llm_url_is_still_local():
    assert resolve_llm_base_url("openai", "http://localhost:11434") == "https://api.openai.com/v1"
    assert resolve_llm_base_url("deepseek", "http://127.0.0.1:11434") == (
        "https://api.deepseek.com/v1"
    )
    assert resolve_llm_base_url("openrouter", "http://localhost:11434") == (
...
def test_online_timeout_does_not_inherit_local_30_minute_wait():
    ...
    assert effective_llm_timeout("openai", 3600) == 120
    assert effective_llm_timeout("openai", 90) == 90
    assert effective_llm_timeout("ollama", 3600) == 3600


def test_local_ollama_timeout_is_not_left_at_hosted_120s():
    assert effective_llm_timeout("ollama", 120) == 3600
    assert effective_llm_timeout("ollama", 1800) == 3600
...
def test_embeddings_provider_is_independent_of_chat(monkeypatch):
...
def test_canonical_llm_api_key_wins_for_any_provider(monkeypatch):
    ...
    assert config.settings.api_key_for_provider("openrouter") == "canonical-online-key"
...
def test_blank_canonical_key_falls_back_to_vendor_alias(monkeypatch):
    ...
    monkeypatch.setattr(config.settings, "OPENROUTER_API_KEY", "or-alias")
    ...
    assert config.settings.api_key_for_provider("openrouter") == "or-alias"
...
def test_default_chat_model_names_for_online_providers():
    ...
    assert default_chat_model("openrouter") == "openai/gpt-4o-mini"
